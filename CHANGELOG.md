...
- **Automatic take profit / stop loss after every buy, plus trailing stops (Settings → Exits).** Off by default; buying more re-bases them on your average entry; legs the price already passed are skipped, never fired. A trailing stop ratchets up with the highest price your coin actually reaches and fills at the real market price when it triggers — never at the level you set.

- **Trade open positions from the dashboard.** Every row now shows the live price and has Sell / Buy / Chart buttons. A dashboard fill needs two independent price sources to agree within 3% and books the less favourable one, so the dashboard can never beat the chart.

- **GMGN: trade prints from thin or stale pools can no longer move the price.** Each print is now tied to the pool it came from; prints from pools other than the dominant one must match its price within 3% while it is still trading, or they are ignored.

- **Fixed: after a take-profit, stop-loss or trailing-stop exit, the next manual buy on that coin could refuse for a few seconds.** The exit's own fill is now counted as fresh market evidence, so re-buying works right away.

Tested: extension 2691/2691, server 387/387, bot 20/20 green (3098 total).
...
- **Chip buy, then open the chart: the entry is there.** Opening the coin mid-fill used to kill the list tab before the bag landed, and even a fill that did persist was keyed by the pair so the chart looked up the mint and drew nothing. The list now stashes the quote with the click; the chart commits it and reloads the wallet before healing stand-in keys, so the line and the entry are on the chart when you land.
