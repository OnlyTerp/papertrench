...
function renderSettings(el) {
  ...
  el.innerHTML = `
    ...
    <div class="field"><label for="set-panel-theme">Panel theme</label><select id="set-panel-theme"><option value="trench" ${!settings.panelTheme || settings.panelTheme === 'trench' ? 'selected' : ''}>Trench — PaperTrench default (amber on slate)</option><option value="lute" ${settings.panelTheme === 'lute' ? 'selected' : ''}>Lute — deep indigo, violet accents</option><option value="solana" ${settings.panelTheme === 'solana' ? 'selected' : ''}>Solana — void green, terminal phosphor</option></select><small>Skins the PaperTrench overlay to match your dex. The site's own page is never touched — only the panel's colors.</small></div>
