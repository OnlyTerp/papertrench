    ...
    <div class="set-panes" id="set-panes">
      ...
      <section class="set-pane" data-group="speed" data-label="Speed">
        ...
        <div class="card">
          ...
          <div class="field field-check"><label><input type="checkbox" id="set-warm-sameterminal" ${settings.warmSameSiteEnabled === true ? 'checked' : ''}> Same-terminal chart opens</label><small>On GMGN, Axiom, Padre, Lute and fomo: hover a coin and a hidden second tab of that terminal loads its chart; the click reveals it once it has finished loading (before that, the click stays native — the viewer is never allowed to be slower than the site). The first click creates the tab; hovers only ever navigate it. From the viewer tab itself, clicks are native.</small></div>
 ...
 * writes nothing back to the DOM, so calling it repeatedly is safe and
...
function gatherSettingsFromForm(notes = [], base = settings) {
  ...
  return {
    ...
    listQuickOpen: document.getElementById('set-list-quick-open').checked,
    ...
    warmSameSiteEnabled: document.getElementById('set-warm-sameterminal').checked,
