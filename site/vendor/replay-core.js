...
function normalizeTrade(item, candleByMinute, mintHint) {
  ...
  if (!mint) return null;
  // The wallet that OWNS the position is the swap sender - the address whose
  // token account the swap credits/debits. transactionSenderAddress is often a
  // bot router / fee payer that holds NO token account at all (verified on
  // chain 8/26: the tape's top "wallet" by tx-sender had zero ATAs for the
  // mint, while its swapSenderAddress held exactly the traded balance).
  const wallet = isAddress(item.swapSenderAddress || item.transactionSenderAddress)
    ? (item.swapSenderAddress || item.transactionSenderAddress) : null;
...
// The SAME file ships to the browser as site/vendor/replay-core.js (a byte-
// identical copy enforced by test/replay-core-sync.test.js), so the client
// chain lane folds fills with the very accounting the server tests prove.
