...
[![Tests](https://img.shields.io/badge/tests-1207%20passing-34D399?style=flat-square)](#tests)
...
```bash
cd extension && node --test    # 1104
cd server   && node --test     #  103
```

**1104 extension tests** covering price resolution, tick validation, portfolio arithmetic, the Padre chart bridge, fresh-launch handling, the positions bar, session replay, the attestation chain, and browser-context loading.

**103 server tests** covering chain verification, re-pricing against market history, ranking, sprint and duel windows, and achievements — the leaderboard verifier in [`server/`](server/), which recomputes standings from submitted chains rather than trusting a submitted number.
