...
- **No account required for paper trading.** Your wallet, trades and journal stay local unless you manually submit a leaderboard record or explicitly enable Tournament sync after joining.
...
- **Network calls:** our own price service (`papertrench-api.onerobby.workers.dev/api/quote`, since v3.22) receives token addresses and chain, with no account or cookies, as an independent price source; the public Dexscreener and Jupiter APIs; public Solana RPCs or your configured private RPC; the venue APIs on supported markets; GitHub release checks (switchable); Daily Spark when you play; X oEmbed for opted-in hover previews; and your own configured AI endpoint. **Tournament sync** is separate and opt-in after joining: while you hold a live seat, the extension sends the same verified fill chain to our API every five minutes and after each cut, with a scoped 30-day Bearer token and no cookies. It is not in backups or debug reports; the dashboard removes the local grant and the tournament page revokes server tokens.
...
- **Leaderboard submission is opt-in.** Manual `papertrench.com` Sync still requires the **Site sync** setting and a user click. Tournament auto-sync is a separate join-time consent; it uses a 30-day token scoped to `/api/submit` and `/api/tournament/mine`, and the extension sends no record until you grant it. No other origin can request or grant sync.
