...
**C-31 · S1 · Axiom's page-level room price was dropped as unnamed JSON content** — `price-bridge.js:376,458-611,696-779,831-993,1062-1098`; `content.js:718-778`; `quote.js:615-654` · axiom.trade/meme · captured; fixed
The field reports say 114/127 Axiom token pages had stale/missing page ticks and 129/142 host-supply reports lacked corroborating USD price/live market cap. In the attached Solana pair pages, the top-level cluster WebSocket sends `b-<pair>` with native price in numeric `content`; `f:<pair>` carries a tuple. The generic collector recognizes named price/mcap/supply keys and address fields, but neither `room` nor scalar `content` supplies one, so the frame exits without a tick. Dossier §4/§5 plus the captured BONK room frames establish the page-level transport. The decoder now exact-matches the current paper-axis pair/mint, gates to `?chain=sol`, accepts numeric `b-` prices, and accepts `f` tuple indices 4/5 only after matching a recent `b` price within 0.5%. Across 35 captured `f` frames, index 4 matched `b` in 35/35 and index 5/index 4 matched the independent SOL/USD room with 0.0084% median relative error. Wrong pairs, non-Solana pages, malformed tuples, stale `f` confirmations, and undecoded room families fail closed. The Eucalyptus SharedWorker stream did not carry the captured token-price rooms; no Axiom SharedWorker tap was added. Supply-only facts continue through the existing accepted-live-quote completion and 1% agreement rule.

**C-32 · S1 · A cap tick converted through the resolver's supply convention minted a price nobody was trading — GMGN PONS flickered ~$0.62 ⇄ ~$0.91 and dips filled for an instant +44%** — `quote.js` (`validateTick` mcap basis, `calibrateChartSupply`); `content.js` (`handlePageTick`, `chartSupplyUi`) · gmgn.ai/robinhood · jbay8976, v3.23.4, ~2026-09-15→18 ("price kept dropping from ~$700M to ~$500M for a second then back"; bought the dips, 1k → 160k) · confirmed live, reproduced on v3.23.4 AND v3.25.0 · **fixed v3.25.1**
GMGN's TradingView symbol is `…/USD/MCAP`: its candles and chart bars are market cap on the ~1B total-supply convention, while the resolver anchor carried ~684M circulating (Dexscreener's own $427M cap). `validateTick`'s mcap basis derived the token price as `anchorPrice × tickMcap/anchorMcap`, so a correct $622M candle printed $0.91 against the true $0.622; interleaved `token_activity` USD ticks then re-accepted at $0.62 — both well inside the 20× accept band and the 3×/15 s scale-step bound, so nothing refused the flip-flop (78 outlier samples in five minutes, GMGN's own display never moved). A disposable-profile replay filled a buy at $0.6228 and sold 3 s later at $0.9104 — +44% booked on a flat market. The fix learns the chart's own supply: `calibrateChartSupply` pairs each cap tick with the freshest accepted direct price (≤3 s) and keeps a 5-sample median of `cap/price`; while calibrated, the mcap basis converts `tickMcap/chartSupply` (still inside the anchor band) and direct ticks restate their cap on the chart's convention so the headline stops oscillating. Average/TP-SL line levels on the MC chart convert through the same learned supply. Without calibration the legacy anchor-ratio path is unchanged (MC-only charts keep working).
...
**F-66 · S1 · `token_activity` dropped the pool id: any pool's trade on the mint could move the accepted price**
`extension/price-bridge.js` (`forwardTokenActivity`); `quote.js` (`notePoolTrade`, `poolConsensusVerdict`); `content.js` (`handlePageTick` pool gate) · gmgn.ai · found during the C-32 investigation · **fixed v3.26.0**
Each `token_activity` item carries the pool in `m` and the USD trade price in `pu`, but the emitted tick kept only the mint (`a`): a trade printed on a thin, stale, or manipulated pool for the same token was accepted as the token's price whenever it landed inside the 20× anchor band (a ~30% print passes every bound). The bridge now forwards `m` as `pool` plus the trade's USD size (`au`) on the tick; `handlePageTick` holds every pool-attributed print to a rolling 60 s per-pool window — the dominant pool (by USD volume, by count when sizes are absent) sets the reference, and any other pool's print must match a fresh (≤10 s) dominant print within 3% or it is dropped into one rolling `pool-consensus-drop` diagnostic. Dropped prints are still noted, so dominance follows real volume migration. Replaying the PONS capture (147 prints across ~98 pools, all ±1%) drops zero prints for disagreement — every drop is the fail-closed stale-reference case, and the chart feeds carry the price meanwhile.
...
**D-73 · S2 · All five fill lanes could be empty at once — a quiet lowcap chart lost the trade, and the only recovery was reloading the page**
`extension/content.js` (pickQuoteForTrade, corroborateForFill, quoteForTradeWithin, doBuy, doSellInner), `extension/quote.js` (AGGREGATOR_SOURCES)

cheng.4848, Discord 🐛-bug-reports: *"I often can't make a purchase in time… generally only happens with low-market-cap coins"*, and ark_trades13 on the exit side: *"I have to refresh the page a few times before I can sell."* The fill ladder has five lanes and every one of them can decline at the same instant — a 1-second lowcap chart between candles, the public Solana pool cooling down under 429s (15,902 publicnode refusals across the v3.22/v3.23.0 exports), an aggregator that has not indexed the pair. Our own price service was already running and already trusted as a WITNESS against a suspicious page price, but it was never allowed to be the SOURCE of one, so the click was dropped with every gate passing except "somebody prove a price". The second half is that a click was treated as one instantaneous sample: a refusal at millisecond zero was final, which is why reloading the page (a fresh sample) "fixed" it and why it read as a lowcap-only defect.

**fixed v3.24.0** (the worker quote becomes a PRIMARY lane in `pickQuoteForTrade` — Solana only, guarded on `typeof R.workerQuote === 'function'`, tagged source `action-worker`, mcapUsd only — judged by the same honesty gates as every other lane, and banned from witnessing itself in `corroborateForFill`; a click is an intent rather than a sample: `quoteForTradeWithin` re-asks for up to `FILL_RETRY_WINDOW_MS` 6000 ms on a `FILL_RETRY_BEAT_MS` 400 ms beat with every gate re-running per attempt, wired into `doBuy` and `doSellInner` so the existing ladder tests keep their single-shot semantics. Locked by `test/workerquote.test.js` — 5 new cases: the dead-chain/dead-resolver/stale-screen fill, the foreign-pair refusal, the self-witness ban, the in-click retry, and the bound that still refuses an unpriceable coin. Negative controls C1–C5: each broke real source, went RED with the expected failure, restored byte-identical by SHA-256, went GREEN.)

---

**D-74 · S3 · A foreign-chain page priced fine on screen and refused every buy, and Axiom's Robinhood pages never mounted at all**
`extension/content.js` (handlePageTick — token.solUsdAtResolve), `extension/sites.js` (AXIOM_CHAIN_BY_SLUG)

Two halves of one gap. A BSC or Robinhood page the resolver never resolved showed a live price from the page's own feed and then refused every order with *"No SOL/USD rate for this chain"*, because only the resolver ever recorded that rate — while the accepted tick already carried both legs (native and USD) and could have supplied it. And Ark, 2026-09-15: *"why doesnt the extension work on Axiom robinhood?"* — the route table knew only the long `robinhood` spelling while Axiom's own chain selector emits **HOOD**, so those pages matched no chain and the overlay never mounted.

**fixed v3.24.0** (an accepted foreign tick records `token.solUsdAtResolve` from its own two legs, and never overwrites a rate the resolver already established; `AXIOM_CHAIN_BY_SLUG` accepts `hood` alongside `robinhood`, while `AXIOM_SLUG_BY_CHAIN` is deliberately left emitting `robinhood` — an outbound slug is an identifier and guessing it needs a real RH URL to settle which spelling Axiom's own links use. Locked by `test/hostfacts.test.js` (behavioural: the rate lands from the tick, and a resolver rate is not overwritten) and a `?chain=hood` row in `test/chainrouting.test.js`. Negative controls C6–C8, each RED then restored byte-identical then GREEN.)

---

**D-75 · S1 · A buy chip could place an order of a different magnitude than the one printed on it**
`extension/content.js` (renderAll → syncPresetUnits, renderPresets, armLimitBuy)

Souly, 2026-09-17: *"for some coins it won't lemme put money on — it picks like a certain amount for me."* Foreign-chain coins denominate in dollars and Solana in SOL, but the chips were only rebuilt at mount: walking from a BSC coin back to a Solana one left DOLLAR chips on a SOL panel, so tapping "$1000" asked the engine for 1000 SOL and was refused on balance — and the opposite walk quietly bought $2 where the trader read 2 SOL. The amount box's own placeholder carried the same stale unit, and arming a limit from a "$100" chip locked 100 SOL.

**fixed v3.24.0** (`syncPresetUnits()` runs from `renderAll`, so the chips, `presetsUsdMode` and the custom box's placeholder all follow the coin currently in front of the trader; `armLimitBuy` converts dollars to SOL before anything locks, using the recorded rate only, and refuses out loud when no rate exists rather than locking a guessed currency. Locked by source pins in `test/quickbuy.test.js` — `els.buyPresets` is unwired in every harness, so the house convention for panel internals applies. Negative controls C9–C10.)

---

**D-76 · S1 · A second window could erase a committed fill, and an armed or cancelled limit order was never guaranteed to reach the wallet at all**
`extension/content.js` (persistStateNow — the unreachable-worker branch, armLimitBuy, cancelLimitBuy, persistSoon)

cheng.4848, 2026-09-15: *"After placing the buy order, I opened this page in a separate window; then, after closing that window, the order disappeared."* Two mechanisms, both reachable from that one action. (1) Every write goes through the worker's serialized compare-and-swap, which refuses a stale base — but `bounded()` reports a TIMEOUT exactly as it reports a death, and opening and closing a window is precisely what puts an MV3 service worker to sleep. The unreachable branch then wrote the view's whole state blind, so a stale view's ~800 ms heartbeat could overwrite a fill it had never seen. Note the seq asymmetry that makes "adopt whatever is ahead" no defence: a view bumps `seq` on every persist ATTEMPT, so a view whose writes all fail still counts upward past the number in storage while its contents fall further behind. (2) Worse, `armLimitBuy` and `cancelLimitBuy` persisted through `persistSoon`, the 800 ms MARK writer — which drops a write when the worker is slow and, when storage is ahead, calls `adoptState(stored)` and discards the very order it was called to save. The panel toasted "Limit buy armed" over a wallet that had never heard of it; a dropped CANCEL is worse still, because the order returns and can fill.
