...
**F-66 · S1 · OPEN — `token_activity` drops the pool id: any pool's trade on the mint can move the accepted price**
`extension/price-bridge.js` (`forwardTokenActivity`) · gmgn.ai · found during the C-32 investigation · open
Each `token_activity` item carries the pool in `m` and the USD trade price in `pu`, but the emitted tick keeps only the mint (`a`): a trade printed on a thin, stale, or manipulated pool for the same token is accepted as the token's price whenever it lands inside the 20× anchor band (nothing validates pool identity, and a ~30% print passes every bound). In the PONS capture ~105 pools traded within ~1% of each other, so this stayed theoretical there — it was NOT the C-32 mechanism — but a token whose pools diverge would feed the panel whichever pool printed last. Fix direction: carry `m` into the tick and prefer the page's primary pair, or weight by pool liquidity, rather than trusting the freshest print.
